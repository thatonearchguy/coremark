...
uart_send_char(char c)
{
    ...
    volatile unsigned int *uart_bb_led_reg = (unsigned int *)0x2000;

    unsigned char i;

    *uart_bb_led_reg = 0x00;
    uart_bb_delay();

    for (i=8; i != 0; --i) {
        if (c & 0x01) *uart_bb_led_reg = 0xFF;
        else *uart_bb_led_reg = 0;
        c >>= 1;
        uart_bb_delay();
    }
